...
namespace DrawUtils
{
	...
	void DrawGrid(const ModelManager &modelManager, const Camera &camera, Direction dir, float width, float gap, float boundsScale)
	{
		float lineWidth = width / camera.GetScale();
		float vpW = camera.GetViewport()[0] * boundsScale / camera.GetScale();
		float vpH = camera.GetViewport()[1] * boundsScale / camera.GetScale();

		Mat4 transformX = Matrix::Scale(Vector3(lineWidth, vpH, 0.f));
		Mat4 transformY = Matrix::Scale(Vector3(vpW, lineWidth, 0.f));

		Mat4 transform = Matrix::Scale(Vector3(lineWidth, vpH, 0.f)) * Matrix::RotationX(-90.f);
		...
		case Direction::RIGHT:
			...
			transformY *= Matrix::RotationY(90.f);
			break;
		case Direction::LEFT:
			axisX = Z;
			axisY = Y;
		...
		case Direction::BACK:
			axisX = X;
			axisY = Y;
			transformX *= Matrix::RotationX(180.f);
			transformY *= Matrix::RotationX(180.f);
			break;
		}
		...
		float startX = (int)((camera.transform.GetPosition()[axisX] - halfScreenUnitsX) / gap) * gap;
		float startY = (int)((camera.transform.GetPosition()[axisY] - halfScreenUnitsY) / gap) * gap;
